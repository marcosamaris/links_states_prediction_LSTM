...
for line in selected_line:  
    ...
    encoder.summary()
#    plot_model(encoder, to_file='minmax_deconv_vae_encoder.png', show_shapes=True)
    ...
    outputs = Cropping2D(cropping=((3, 0), (1, 0)), data_format=None)(x) # this is the added step
    
    
    # instantiate decoder model
    decoder = Model(latent_inputs, outputs, name='decoder')
    decoder.summary()
#    plot_model(decoder, to_file='minmax_deconv_vae_decoder.png', show_shapes=True)
    ...
    vae.summary()
#    plot_model(vae, to_file='minmax_deconv_vae.png', show_shapes=True)
    ...
    MAE_decode = []
    MAE_encode = []
    test_norm = np.reshape(test_norm, (number_test_samples*18 -1, input_dim_x*input_dim_y))  # adapt this if using `channels_first` image data format
    ...
    for ml in ML_alg:
        ...
        MAE_encode.append(mean_absolute_percentage_error(test_y, y_pred))
            
        decoded_states = decoder.predict(y_pred)        
        
        decoded_states = np.reshape(decoded_states, (number_test_samples*18-1, input_dim_x* input_dim_y))  # adapt this if using `channels_first` image data format
        
        MAE_decode.append(mean_absolute_percentage_error(decoded_states, test_norm))

    x = np.arange(8)
    plt.figure(0)
    plt.bar(x, MAE_encode)
    plt.xticks(x, (ML_alg))
    plt.savefig('MAE_encode_minmax_deconv_' + line + '.png')
    ...
    plt.xticks(x, (ML_alg))
    plt.savefig('MAE_decode_minmax_deconv_' + line + '.png')
